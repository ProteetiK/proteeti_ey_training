...
print("ChatGroq ready: llama-3.3-70b-versatile")
...
def rag_stream(query: str, top_k: int = TOP_K) -> None:
    """Retrieve context then stream Groq response token-by-token."""
    ...
    # Stream response
    for chunk in direct_client.stream(messages):
...
# ============================================================
# LATENCY VARIATION + QUALITY ANALYSIS
# ============================================================
...
def faithfulness_score(answer, context):
    ...
    Replace with Ragas or LLM judge later.
    """
...
for i in range(len(TEST_QUESTIONS)):
    ...
    # --------------------------------------------------------
    # Use full-context answer as pseudo-reference
    # --------------------------------------------------------

    if "fc_results" in globals():
        reference = fc_results[i].answer
    else:
        reference = rag.answer
    ...
    elif hasattr(rag, "retrieved_context"):
        context = rag.retrieved_context
    ...
    rows.append({
        ...
        "retrieve_ms": rag.retrieve_ms,
        "generate_ms": rag.generate_ms,
        "total_ms": total_ms,
        "bleu": bleu,
        "rougeL": rouge,
        "faithfulness": faithfulness,
        "answer_len": len(prediction)
    })
...
ax.axhline(
    ...
    linestyle="--",
    linewidth=2,
    label=f"Avg={results.total_ms.mean():.0f} ms"
)

ax.set_title("Latency Waterfall")
ax.set_ylabel("Latency (ms)")
ax.set_xticks(x)
ax.set_xticklabels(
    [f"Q{i+1}" for i in range(len(results))],
    rotation=45
)
ax.legend()
...
ax.set_title("Latency vs ROUGE-L")
ax.set_xlabel("Total Latency (ms)")
...
ax.set_title("Latency vs Faithfulness")
ax.set_xlabel("Total Latency (ms)")
ax.set_ylabel("Faithfulness")

# ------------------------------------------------------------
# Retrieval Latency vs Faithfulness
# ------------------------------------------------------------
...
ax.set_title("Retrieval Latency vs Faithfulness")
...
plt.suptitle(
    "RAG Latency Variation vs Quality Metrics",
    fontsize=16,
    fontweight="bold"
)
...
plt.savefig(
    "rag_latency_quality_analysis.png",
    dpi=300,
    bbox_inches="tight"
)
...
print(type(rag_results[0]))
print(vars(rag_results[0]))
...
results = pd.DataFrame({
    ...
    "embed_ms": embed_times,
    "retrieve_ms": retrieve_times,
    "generate_ms": generate_times,
    "bleu": bleu_scores,
    "rougeL": rouge_scores,
...
if "context_lengths" in globals():
    results["context_len"] = context_lengths

if "answer_lengths" in globals():
    results["answer_len"] = answer_lengths
...
avg_total = results["total_ms"].mean()

ax.axhline(
    avg_total,
    linestyle="--",
    linewidth=2,
    label=f"Avg = {avg_total:.0f} ms"
)
...
ax.set_xticks(x)
ax.set_xticklabels(
    [f"Q{i+1}" for i in range(len(TEST_QUESTIONS))],
    rotation=45
)
ax.legend()
...
# ============================================================
# (5) Retrieval Cost vs Faithfulness
# ============================================================
...
print("\nSaved: rag_latency_quality_analysis.png")
